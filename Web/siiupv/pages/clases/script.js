...
const maestro = document.querySelector('#select2_maestro');
...
carrera.addEventListener('change',getMaestros);
...
function getMaterias(e){    
    ...
    const tipo = '2';
    const plan = document.querySelector('#select2_plan').value;    
    //var plan = document.getElementById('select2_plan').value;
    console.log('Entre a materias Plan elegido ',plan);
...
function getMaestros(e){    
    ...
    const datos = new FormData();
        datos.append('id',id);        
        console.log("cambio en maestros tipo " + tipo);
        //llama a AJAX
        //crea objeto
        const xhr = new XMLHttpRequest();
        //abrir conexion
        ...
        xhr.onload = function() {
            if(this.status===200){                
                var respuesta = '';
                var mpos='';
                const div = document.querySelector('#select2_maestro');
                try {
                    //console.log(xhr.responseText);
                    respuesta = JSON.parse(xhr.responseText);
